...
if show_threshold_line:
    fig.add_hline(
        ...
        line_dash="dash",
        line_color="#FF4B4B",
        annotation_text=f"Warning Threshold ({mw_warning} MW)"
    )

fig.update_layout(
    xaxis=dict(title="Operational Time (Hours)", range=[0, 24] if time_horizon == "Last 24 Hours" or time_horizon == "All Data" else None),
    yaxis=dict(title="Power (MW)", range=[0, max_y_axis]),
    height=480,
